    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item active"><a href="{{ url()->current() }}">Edit Pos</a></div>
            </div>
        </div>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('pos.update', $pos->id) }}" method="post" id="editPosForm">
                        @csrf
                        @method('put')
                    <div class="row">
                        <div class="form-group col-md-6 col-12">
                            <label>Nama</label>
                            <input type="text" class="form-control" name="edit_nama" id="editName"
                                placeholder="Nama Pos" value="{{ $pos->nama }}">
                        </div>
                        <div class="form-group col-md-6 col-12">
                            <label for="edit_alamat">Alamat</label>
                            <input type="text" class="form-control" name="edit_alamat" id="editAlamat"
                                placeholder="Alamat Pos" value="{{ $pos->alamat }}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-md-6 col-12">
                            ...
                            <select class="form-control" name="edit_jenis_pos" style="color:#6c757d !important;">
                                ...
                                <option value="pos_pelayanan"
                                    {{ $pos->jenis_pos == 'pos_pelayanan' ? 'selected' : '' }}>Pos Pelayanan
                                </option>
                                <option value="pos_terpadu"
                                    {{ $pos->jenis_pos == 'pos_terpadu' ? 'selected' : '' }}>Pos Terpadu</option>
                                <option value="pos_pengaman"
                                    {{ $pos->jenis_pos == 'pos_pengaman' ? 'selected' : '' }}>Pos Pengaman</option>
                            </select>
                        </div>
                        <div class="form-group col-md-6 col-12">
                            ...
                            <select class="form-control" name="edit_district_id" style="color:#6c757d !important;">
                                ...
                                @foreach ($districts as $districts_data)
                                    <option value="{{ $districts_data->id }}"
                                        {{ $districts_data->id == $pos->district_id ? 'selected' : '' }}>
                                        {{ $districts_data->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="card-footer text-right">
                        ...
                        <button type="submit" class="btn btn-primary" id="editPosButton">Simpan Perubahan</button>
                    </div>
                </form>
                </div>
            </div>
